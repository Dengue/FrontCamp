class NewsReader {
	constructor(view,template,observer) {
	...
	events() {
		this.observer.addListener(this,'category-changed',this.fetchNews);
		this.observer.addListener(this,'update-news',this.updateView);
	};
	fetchNews(category) {
	...
	fillNews({num_results,results}) {
		...
		this.observer.emit('update-news',newsColumns);
	};
	updateView(news) {
		this.view.innerHTML = '';
